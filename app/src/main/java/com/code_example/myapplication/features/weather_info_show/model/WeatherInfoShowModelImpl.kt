...
import com.code_example.myapplication.network.ApiInterface
import com.code_example.myapplication.network.RetrofitClient
...
class WeatherInfoShowModelImpl(private val context: Context): WeatherInfoShowModel{
    override fun getCityList(callback: RequestCompleteListener<MutableList<City>>) {
        try {
            val steam = context.assets.open("city_list.json")
            val size =  steam.available()
